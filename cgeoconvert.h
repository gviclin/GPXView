#ifndef CGEOCONVERT_H
#define CGEOCONVERT_H
...
public:
    ...
    Ellipsoid(int id, const char* name, double radius, double fr){
        Id = id;Name=name;  EquatorialRadius=radius;  eccSquared=2/fr-1/(fr*fr);
    }
    const char* Name;
...
public:
    ...
    Datum(int id, const char* name, int eid, double dx, double dy, double dz){
        Id=id; Name=name;  eId=eid;  dX=dx;  dY=dy;  dZ=dz;
    }
    const char* Name;
    int   eId;
    double dX;
...
public:
    ...
    static void DatumConvert(int dIn, double LatIn, double LongIn, double HtIn, int dTo,  double& LatTo, double& LongTo, double& HtTo);
    static void LLtoUTM(int eId, double Lat, double Long,  double& Northing, double& Easting, int& Zone);
    static void UTMtoLL(int eId, double Northing, double Easting, int Zone,  double& Lat, double& Long);
};
